...
void HandleDrawAction(AppState* state) {
    ...
    state->grid[state->previewX][state->previewY].resistance = 0;
}
...
void HandleEditAction(AppState* state) {
    if (!state->isEditing && state->grid[state->previewX][state->previewY].type != COMPONENT_EMPTY) {
        ...
        state->editValue = state->grid[state->editX][state->editY].resistance; // Load current value

        // Initialize text buffer with the current value
        snprintf(state->valueText, sizeof(state->valueText), "%f", state->grid[state->editX][state->editY].resistance);
    }
}